...
// URL: https://codeforces.com/contest/2266/problem/E
// Memory Limit: 256 MB
// Time Limit: 2000 ms
// 
...
void solve()
{
	...
	auto dfs=[&](auto&& self,int x) -> void {
		if (x <= k) {mp[x]=0;return;}
		if (mp.count(x)) return;
		auto v=sieve.fact(x);
		int res=inf;
		for(auto [num,_] : v) {
			if (!mp.count(x/num)) self(self,x/num);
			res=min(res,1+num*mp[x/num]);
		}
		mp[x]=res;
	};
